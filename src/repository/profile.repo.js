...
export async function getProfileDetail(id) {
    const res = await pool.query(`
        SELECT "profile"."id_user", "profile"."fullname", "profile"."username", 
        "profile"."phone", "users"."email" AS "email", "profile"."created_at", "profile"."updated_at", 
        "cart"."id" AS "id_cart", "cart"."created_at" AS "cart_created", "cart"."updated_at" AS "cart_updated" 

        FROM "profile" 
        LEFT JOIN "cart" ON "cart"."id_user" = "profile"."id_user" 
...
export async function updateProfile(id, data){
    ...
    try{
        ...
        const res = await client.query(`UPDATE "profile" SET ${queries}, updated_at = NOW() 
            WHERE id_user=$${queries.length+1}`,[...val, id])
